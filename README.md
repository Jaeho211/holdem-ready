...
- 설정 모달: 언어, 진동, 사운드, 하루 목표, JSON 백업 내보내기/불러오기
- PWA 메타데이터와 서비스 워커 기반 기본 오프라인 캐시
- 개인정보처리방침, 지원 안내 정적 페이지
...
기본 QA는 작은 폰, 큰 폰, 태블릿 세로 뷰포트를 모두 검사합니다.
...
app/
  _components/
    ...
    holdem-quiz-app.tsx     상태 로딩, 저장, 액션 처리
    holdem-quiz-model.ts    앱 상태/액션 타입
    holdem-quiz-view.tsx    화면 조합과 파생 데이터 계산
  ...
  privacy/page.tsx          개인정보처리방침 정적 페이지
  page.tsx                  앱 엔트리 포인트
  qa/ui/page.tsx            개발 전용 QA 프리뷰 라우트
  support/page.tsx          지원 안내 정적 페이지
lib/
  holdem/
...
- [Android 출시 가이드](docs/android-release.md)
